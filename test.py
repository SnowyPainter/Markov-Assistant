...
def test():
    df = data.create_dataset(mo1_5m_long_nvda_tickers, start=data.today_before(data.days_to_years(3)), end=data.today(), interval='1d')
    df = models.lstm_prepare_dataset(df)
    models.test(df.values)
...
def get_tomorrow_updown(symbols):
    ...
    lstm.compile()
    lstm.fit(train_values, 100, 10, './models/lstm.h5', skip_if_exist=True)

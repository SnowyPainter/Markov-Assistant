...
class WSMonitorStock(QThread):
    ...
    def run(self):
        ...
        if self.timezone == TIMEZONE_KRX:
            code_list = [['1', KOREA_ASKINGPRICE_ID, symbol_tr_key], ['1', KOREA_DEALPRICE_ID, symbol_tr_key],['1', KOREA_TRANSCATION_NOTICE_ID, hts_id]]
        else:
        ...
        while self.running:
            ...
            try:
                ...
                if result["error"] == -1:
                    break
                if result["type"] == HANDLE_ASKINGPRICE: #호가 처리
                    df = create_realtime_dataset_by_price(self.symbol, float(result["predicted_price"]), self.timezone)
                    self.monitor.env.append_raw(df)
                    self.signal.emit(tradeinfo.asking_price_info(result, self.timezone))
                    self.msleep(int(500))
